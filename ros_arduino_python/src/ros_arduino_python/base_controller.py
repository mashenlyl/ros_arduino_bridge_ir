...
class BaseController:
	...
	self.SensorPos[0] = 30.0/180.0*3.14           # sensor position    
  	...
  	self.SensorPos[2] = 150.0/180.0*3.14
  	self.SensorPos[3] = 210.0/180.0*3.14
  	self.SensorPos[4] = 270.0/180.0*3.14
  	self.SensorPos[5] = 330.0/180.0*3.14
...
#            self.enc_left = left_enc
            ...
            self.df_vx = ((2 * dwheel2 - dwheel1 - dwheel3) / 3) * dt
	    self.df_vy = ((dwheel3 - dwheel1) * 0.57735) * dt
	    self.df_vth = ((dwheel1 + dwheel2 + dwheel3) / (3 * self.wheel_track)) * dt

	    delta_x = (self.df_vx * cos(self.th) - self.df_vy * sin(self.th))
            delta_y = (self.df_vx * sin(self.th) + self.df_vy * cos(self.th))
            delta_th = self.df_vth
...
#                )
	    ...
	    odom.pose.pose.orientation.w = quaternion[3]
            odom.twist.twist.linear.x = self.df_vx
            odom.twist.twist.linear.y = self.df_vy
            odom.twist.twist.angular.z = self.df_vth
...
#        self.arduino.drive(0, 0)
	...
	for i in range(0,6) :
    	    self.IRdistance[i]= GetIRdistance(self.IRdistance[i])
    	    if self.IRdistance[i] <= 15 :
    	        self.IRdistance[i] = (20/self.IRdistance[i]) * (20/self.IRdistance[i])
    	    else
		self.IRdistance[i] = 0
    	    IRSetspeedX = IRSetspeedX-  self.IRdistance[i] * float(self.KIRp) * cos(self.SensorPos[i]) - float(self.KIRd) * cos(self.SensorPos[i]) * float(self.df_vx) / sqrt((cos(self.SensorPos[i]))**2 + (sin(self.SensorPos[i]))**2 )
    	    IRSetspeedY = IRSetspeedY-  self.IRdistance[i] * float(self.KIRp) * sin(self.SensorPos[i]) - float(self.KIRd) * sin(self.SensorPos[i]) * float(self.df_vy) / sqrt((cos(self.SensorPos[i]))**2 + (sin(self.SensorPos[i]))**2 )
    

        wheel1 = -0.5 * (x + IRSetspeedX) - 0.86603 * (y + IRSetspeedY) + th * self.wheel_track  * self.gear_reduction
        wheel2 = (x + IRSetspeedX) + th * self.wheel_track  * self.gear_reduction
        wheel3 = -0.5 * (x + IRSetspeedX) + 0.86603 * (y + IRSetspeedY) + th * self.wheel_track  * self.gear_reduction
