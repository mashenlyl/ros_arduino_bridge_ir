...
#            return values
    ...
    def drive(self, wheel1, wheel2, wheel3):
        ''' Speeds are given in encoder ticks per PID interval
        '''
        return self.execute_ack('m %d %d %d' %(wheel1, wheel2, wheel3))
    
    def drive_m_per_s(self, wheel1, wheel2, wheel3):
        ''' Set the motor speeds in meters per second.
        '''
        wheel1_revs_per_second = float(wheel1) / (self.wheel_diameter * PI)
        wheel2_revs_per_second = float(wheel2) / (self.wheel_diameter * PI)
        wheel3_revs_per_second = float(wheel3) / (self.wheel_diameter * PI)
        
        wheel1_ticks_per_loop = int(wheel1_revs_per_second * self.encoder_resolution * self.PID_INTERVAL * self.gear_reduction)
        wheel2_ticks_per_loop  = int(wheel2_revs_per_second * self.encoder_resolution * self.PID_INTERVAL * self.gear_reduction)
        wheel3_ticks_per_loop  = int(wheel3_revs_per_second * self.encoder_resolution * self.PID_INTERVAL * self.gear_reduction)
        self.drive(wheel1_ticks_per_loop , wheel2_ticks_per_loop, wheel3_ticks_per_loop )
